...
public class SkeletonColliders : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D other)
    {
        ...
        foreach (GameObject g in gameObjects)
        {
            if (!g.GetComponent<SkeletonManager>().SkeletonIsDead)
            {
                ...
                detectionColliderDetected = skeletonMovement.DetectionColliderObject.GetComponent<Collider2D>().IsTouching(other);
        ...
        foreach (GameObject g in gameObjects)
        {
            if (!g.GetComponent<SkeletonManager>().SkeletonIsDead)
            {
                if (g != null)
                {
                    ...
                    {
                        if (skeletonMovement.DetectionColliderObject && skeletonMovement.AttackColliderObject)
                        {
                            detectionColliderDetected = skeletonMovement.DetectionColliderObject.GetComponent<Collider2D>().IsTouching(other);
    ...
    public void TriggerDetection(GameObject g)
    {
        ...
        if (skeletonMovement != null)
        {
            skeletonMovement.PlayerDetected();
        }
    }
}
